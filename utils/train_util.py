...
###Training utils
###
def find_class_by_name(name, modules):  #('AdamOptmizer', [tf.train])
    """Searches the provided modules for the named class and returns it."""
    modules = [getattr(module, name, None) for module in modules]  # mudule = tf.train,  getattr函数用于返回一个对象属性值//返回tf.train的名叫AdamOptmizer的属性--> modules = [tf.train.adamoptimizer]
    return next(a for a in modules if a)  # next返回迭代器的下一个值-->初始值是index=0，调用一次next得到首单元，而a for a in modules if a 本身构成一个迭代器-->返回tf.train.adamoptimizer
...
def get_tag_id_dict(tag_id_file):  # input: string 指label_id.txt
    tag_id_dict={}
    with open(tag_id_file, 'r') as lnf:
        for line in lnf:
            tag, idx = line.strip().split('\t')  # 文件中tag和id之间用制表符分隔
            tag_id_dict[tag] = int(idx)  # 保存到字典中
    return tag_id_dict
