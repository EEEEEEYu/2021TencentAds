...
from tensorflow.compact.v1 import logging
...
class Trainer(object):
    def __init__(self, cluster, task, model, reader, configs):
        ...
        self.modal_name_list = []  # ['video', 'audio', 'text', 'image']
    ...
    def build_train_graph(self):
        ...
        optimizer_dict ={}
        optimizer_class = train_util.find_class_by_name(self.optimizer_config.optimizer, [tf.train])  # (name, modules)-->('AdamOpt')-->optimizer_class = tf.train.adamoptimizer
        for k,v in learning_rate_dict.items():  # learning_rate_dict={'video':float, 'audio':float, 'text':float, 'image':float, 'classifier':float}  k='video' v=float
            if k not in self.modal_name_list+['classifier']: #支持不同模块的学习率独立设置  ['video', 'audio', 'text', 'image', 'classifier'] 检测异常(优化器与模型不匹配)
                print("key:" ,k)
            ...
            optimizer_init_params = self.optimizer_config.optimizer_init_params.copy()  # 从配置文件中加载，{}
            optimizer_init_params['learning_rate'] = learning_rate_dict[k]  # 当前模块的学习率
            optimizer_dict[k] = optimizer_class(**optimizer_init_params)  # 承自代码90&91，为每个模块的adam优化因子设置了学习率
        ...
        print('\n开始遍历num_towers\n')
        print('num_towers:{}'.format(num_towers))
        ...
        print('merged_gradients:{}'.format(merged_gradients))
        ...
        print('遍历merged_gradients,依次打印每一个元素的var.name')
        ...
        print('\n训练图模态及分类器信息显示完毕，开始处理训练单元train_op\n')
        train_op_list=[]
    ...
    def run(self, config_path):
        ...
        with tf.get_default_graph().as_default() as graph:
            ...
            with tf.device(device_fn):
                ...
                saver = self.build_model()
                print('\nsaver=build_model执行完毕，开始初始化global_variables\n')
                global_init_op = tf.global_variables_initializer()
                print('\nglobal_variables初始化完毕，开始初始化dataset,执行tf.data.Dataset.from_generator().make_initializable_iterator().initializer\n')
                dataset_init_op = self.reader.iterator.initializer
                #init_op = tf.group([global_init_op, dataset_init_op])
        print('\n训练图构建完毕，开始执行tf.train.Supervisor\n')
                ...
                while not self.sv.should_stop():
                    try:
                        ...
                        if self.is_master and train_fetch_dict_eval['global_step'] % 10 == 0 and self.optimizer_config.train_dir:
                            ...
                            if self.is_master and time_to_export:  # 验证
...
def train_main(config_path, TrainerType):
    print('\nbase_trainer部分开始\n')
    config = yaml.load(open(config_path))  # 加载配置文件
    print('\n打印配置文件\n')
    print(config)

    env = json.loads(os.environ.get("TF_CONFIG", "{}"))  # 获取关键字为TF_CONFIG的环境变量，存为json文件
    ...
    if not cluster or task.type == "master" or task.type == "worker":
        print('\n加载模型，执行models.get_instance语句\n')
        model = models.get_instance(config['ModelConfig']['model_type'],
                                    config['ModelConfig'])
        print(model)
        print('\n加载数据，执行dataloader.data_pipeline语句\n')
        reader = dataloader.Data_Pipeline(config['DatasetConfig'])  # reader是一个Pipeline类
        print('\n模型与数据加载完毕，开始执行train_main部分sess.run\n')
